...
public class HUDHandlerFakeBlock implements IComponentProvider {
    ...
    public void appendHead(List<ITextComponent> tip, IDataAccessor accessor, IPluginConfig config) {
        ...
        tooltip.setTag(PluginFakeBlocks.OBJECT_NAME_TAG, new StringTextComponent(String.format(Waila.CONFIG.get().getFormatting().getBlockName(), block.getBlock().getNameTextComponent().getFormattedText())));
